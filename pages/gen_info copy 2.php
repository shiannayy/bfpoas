<?php include_once "../includes/_init.php";
   ...
   if (!isset($_SESSION['gen_info_id'])) {
    // Insert draft row
    $data = ["status" => "draft", "created_at" => date("Y-m-d H:i:s")];
    $gen_info_id = insert_data("general_info", $data);
    $_SESSION['gen_info_id'] = $gen_info_id;
} else {
    $gen_info_id = $_SESSION['gen_info_id'];
}

// Fetch row
$general = select("general_info", ["gen_info_id" => $gen_info_id], null, 1);
    ...
    <div class="container" style="margin-top:75px; margin-bottom:50px;">
        <form id="generalInfoForm">
           <h3 class="mb-1 text-navy">General Information
            <button type="button" id="newGeneralInfo" class="btn btn-navy shadow float-end">+ New</button>
            <button type="reset" id="newGeneralInfo" class="btn btn-gold shadow mx-2 float-end">Reset</button>
        ...
        <span class="small mb-2 badge text-bg-info fw-normal">This form has auto-save feature.</span>
            ...
            <div class="text-end">
                <button type="submit" class="btn btn-success">Save General Info</button>
            </div>
        </form>
        ...
        $(document).ready(function() {
            ...
            function autoSave($input) {
                ...
                $.post("../includes/auto_save_general_info.php", formData, function(res) {
                    ...
                    <small class="text-muted">Saved at ${new Date().toLocaleTimeString()}</small>
                `);
                    } else {
            ...
            $("#generalInfoForm").on("submit", function(e) {
                e.preventDefault();
                $("#generalInfoForm .auto-save").each(function() {
                    autoSave($(this));
                });
            });
        });
