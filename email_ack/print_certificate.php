...
if(isset($_GET['inspection_id']) && isset($_GET['token'])){
    $token = mysqli_real_escape_string($CONN, $_GET['token']);
    $inspection_id = mysqli_real_escape_string($CONN, $_GET['inspection_id']);
    //check token
    $tokenExists = select('email_token', ['email_token' => $token], null , 1);
    if(empty($tokenExists)){
        header("location: ../public/?NotRecognized.");
        die();
    }
    else{
        //check if fully approved
        //1. check the inspections table.
        $schedId = $tokenExists[0]['schedule_id'];
        $approved = select('inspections',['schedule_id'=>$schedId, 'inspection_id' => $inspection_id, 'hasFinalApproval' => 1],null,1)[0];
        if(empty($approved)){
            header("location: ../public/?NotYetApproved.");
            die;

        }
        else{
            //mark as sent
            update_data("inspections", 
                [
                    "cert_sent" => 1,
                    "cert_sent_at" => date('Y-m-d H:i:s')
                ], 
